import java.util.ArrayList;
import java.util.List;

class Solution1 {

    public int minimumMoves(int[][] grid) {

        List<int[]> extraStones = new ArrayList<>();
        List<int[]> emptyCells = new ArrayList<>();

        // Every cell containing more than one stone
        // contributes its extra stones,
        // while every zero-valued cell
        // becomes a destination.
        for(int row=0; row<3; row++) {
            for(int col=0; col<3; col++) {
                int stones = grid[row][col];

                if(stones==0) emptyCells.add(new int[]{ row, col });

                // Store every extra stone separately
                // because each one can be assigned
                // to a different empty cell.
                for(int count=1; count<=stones-1; count++) {
                    extraStones.add(new int[] { row, col });
                }
            }
        }

        int n = extraStones.size();
        int m = emptyCells.size();

        // Every bit represents
        // whether the corresponding empty cell
        // is still unfilled.
        int emptyCellsMask = (1<<m)-1;

        // memo[i][mask]:
        // stores the minimum remaining cost
        // after assigning stones
        // starting from index i,
        // while the empty cells represented
        // by 'mask' are still available.
        Integer[][] memo = new Integer[n][emptyCellsMask+1];

        return recursion(extraStones, emptyCells, 0, emptyCellsMask, memo);
    }

    private int recursion(List<int[]> extraStones, List<int[]> emptyCells, int i, int emptyCellsMask, Integer[][] memo) {

        // Every extra stone
        // has already been assigned.
        if(i==extraStones.size()) return 0;

        // Return previously computed state.
        if(memo[i][emptyCellsMask]!=null) return memo[i][emptyCellsMask];

        int minCost = Integer.MAX_VALUE/2;

        // Try assigning
        // the current extra stone
        // to every still-empty cell.
        for(int bit=0; bit<emptyCells.size(); bit++) {

            int bitValue = (emptyCellsMask>>bit)&1;

            if(bitValue==1) {

                // Remove this empty cell
                // from the available set
                // because it becomes occupied.
                int newEmptyCellsMask = emptyCellsMask^(1<<bit);

                // Cost equals
                // the Manhattan distance
                // travelled by the stone.
                int cost = calcCost(extraStones, emptyCells, i, bit);

                minCost = Math.min(
                    minCost,
                    cost + recursion(extraStones, emptyCells, i+1, newEmptyCellsMask, memo)
                );
            }
        }

        memo[i][emptyCellsMask] = minCost;

        return minCost;
    }

    private int calcCost(List<int[]> extraStones, List<int[]> emptyCells, int i, int j) {

        int x1 = extraStones.get(i)[0];
        int y1 = extraStones.get(i)[1];

        int x2 = emptyCells.get(j)[0];
        int y2 = emptyCells.get(j)[1];

        // Every move changes either
        // the row or column by one,
        // so the minimum moves required
        // equal the Manhattan distance.
        return Math.abs(x1-x2) + Math.abs(y1-y2);
    }
}

class Solution2 {

    public int minimumMoves(int[][] grid) {

        List<int[]> extraStones = new ArrayList<>();
        List<int[]> emptyCells = new ArrayList<>();

        // Collect every extra stone
        // and every empty destination.
        for(int row=0; row<3; row++) {
            for(int col=0; col<3; col++) {
                int stones = grid[row][col];

                if(stones==0) emptyCells.add(new int[]{ row, col });

                // Store each extra stone separately
                // because assignments
                // are made individually.
                for(int count=1; count<=stones-1; count++) {
                    extraStones.add(new int[] { row, col });
                }
            }
        }

        int n = extraStones.size();
        int m = emptyCells.size();

        // Initially,
        // every empty cell
        // is still available.
        int emptyCellsMask = (1<<m)-1;

        // dp[i][mask]:
        // stores the minimum remaining cost
        // after assigning stones
        // starting from index i,
        // while the empty cells in 'mask'
        // are still unfilled.
        int[][] dp = new int[n+1][emptyCellsMask+1];

        // Build states backwards
        // because every transition
        // depends only on dp[i+1][...].
        for(int i=n-1; i>=0; i--) {

            for(int mask=1; mask<=emptyCellsMask; mask++) {

                int minCost = Integer.MAX_VALUE/2;

                // Try assigning
                // the current stone
                // to every available empty cell.
                for(int bit=0; bit<m; bit++) {

                    int bitValue = (mask>>bit)&1;

                    if(bitValue==1) {

                        // Mark this destination
                        // as occupied.
                        int newMask = mask^(1<<bit);

                        int cost = calcCost(extraStones, emptyCells, i, bit);

                        minCost = Math.min(
                            minCost,
                            cost + dp[i+1][newMask]
                        );
                    }
                }

                dp[i][mask] = minCost;
            }
        }

        return dp[0][emptyCellsMask];
    }

    private int calcCost(List<int[]> extraStones, List<int[]> emptyCells, int i, int j) {

        int x1 = extraStones.get(i)[0];
        int y1 = extraStones.get(i)[1];

        int x2 = emptyCells.get(j)[0];
        int y2 = emptyCells.get(j)[1];

        // Minimum moves between two cells
        // equal their Manhattan distance.
        return Math.abs(x1-x2) + Math.abs(y1-y2);
    }
}